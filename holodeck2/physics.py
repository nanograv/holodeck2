...
import numpy as np
from . import cosmo
from .constants import NWTG, SPLC, TWO_PI, YR


_GW_CONST = (64.0 / 5.0) * np.power(NWTG, 3.0) / np.power(SPLC, 5.0)
# [Sesana2004]_ Eq.36
_GW_SRC_CONST = (8.0 / np.sqrt(10.0)) * np.power(NWTG, 5.0/3.0) * np.power(np.pi, 2.0/3.0) / np.power(SPLC, 4)
_GW_DADT_SEP_CONST = - (64.0 / 5.0) * np.power(NWTG, 3) / np.power(SPLC, 5)
_GW_DEDT_ECC_CONST = - (304.0 / 15.0) * np.power(NWTG, 3) / np.power(SPLC, 5)
# [EN2007]_, Eq.2.2
_GW_LUM_CONST = (32.0 / 5.0) * np.power(NWTG, 7.0/3.0) * np.power(SPLC, -5.0)
# [EN2007]_, Eq.2.9
_GW_TAU_CONST = (5.0 / 96.0) * np.power(NWTG, -5.0/3.0) * np.power(SPLC, 5.0) * np.power(TWO_PI, -8.0/3.0)
